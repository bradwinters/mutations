####################################################
#   Problem 4, chapter 1, Bioinformatics VI finding
#   mutations.
#   Multiple pattern matching, using suffix tree, 
#   and Longest Repeat Problem
#   Input: ATATCGTTTTATCGTT
#   Output: TATCGTT
#   ---------------------------------------------
#
#   Use the suffix tree, which has noted mutliple symbols
#   per node already.  Traverse the Tree, look for fully
#   duplicated paths, from root down to end of the edge.
#
####################################################
...
##################################################
#  Simply reverse the list and only print chars
#     of nodes
##################################################
...
def backTrack(aNode):
    ##### given a node, backtrack all single child nodes 
    ##### creating a list to return
    ##### stop when a multi child node is encountered
    ##### which will possibly include root
    #####
    # lizard
    rList=[]
...
def backTrack2(aNode):
    ...
    ##### which will possibly include root
    #####
    ##  Differs here from backtrack in that the start node is 
    ##  not concidered
    rList=[]
    rList.append(aNode)
    #  move up to the parent of this node, pnode
    aNode=aNode.pnode
    x=len(aNode.children)
    while (len(aNode.children) <= 1):
       #print("backtracking...")
...
def ModifiedSuffixTrieConstruction(Text):
    ...
    # add routine will reuse starting characters of each suffix as much as it can
    # when each successive suffix differs, it branches off at that point
    # add is key routine that notes where prefixes overlap and suffixes split off
    while len(Text) >= 1:  #  Text will get shorter each loop as it progresses
       add(root,Text)
       Text=Text[1:]
       textPos+=1
...
def add(root, word: str):
    ...
    for char in word:
        ...
        if not found_in_child:
            new_node=TrieNode(char,node,node.id)
    ...
    #####################################################
    #   Second ModifiedSuffixTrieConstruction
    #    This parallels the first, but only tracks largest
    #    LCS between strings.  No Trie is built or altered
    #    just threads the prefix, keeps the longest
    #    calls its own version of add which should be called
    #    find largest
    #####################################################
def MSTC(root,Text):
    #create the root by constructing the root node, parent of itself
    ...
    while len(Text) >= 1:  #  Text will get shorter each loop as it progresses
       #print(Text)
       preFix=findLargestSS(root,Text)
       #print("Back from findLargestSS, compare ",LCSS," to ",preFix)
       if len(preFix) >= len(LCSS):
           LCSS=preFix
       Text=Text[1:]
    return LCSS 


def findLargestSS(root, word: str):
    node=root # the Trie root
    preFixPath=[] # return matching nodes
    for char in word:
        found_in_child=False
        # search for the char
        for child in node.children:
            if child.char==char:  # a match, follow the child
                #print("Match Trie char: ",child.char," Text ",char)
                #found it
                child.counter+=1
                #point the node to the child
                #continue down this path
                node=child
                found_in_child=True
                preFixPath.append(child.char)
                break
        if not found_in_child:
            #print("No match, quit")
            return preFixPath
    #print("Returning ",preFixPath)
    return preFixPath
...
if __name__ == "__main__":
    ...
    ## First build a normal Trie from line1
    ## then call a similar routine the follows the same logic
    ## of continuing to compare and find matching prefixes, but 
    ## the second process does not add to Trie, it just tracks
    ## the longest common substring bewteen the two
    ## To do this, an altered copy of ModifiedSuffixTrieConstruction(line1)
    ## will be used, call it MSTC.  MSTC accepts the Trie built in 
    ## the first step as well as line2.
    Trie=ModifiedSuffixTrieConstruction(line1)
    #print("Trie built, find ",line2)
    LCS=MSTC(Trie,line2)
    #print("Answer is ")
    printList(LCS)
    #print()
